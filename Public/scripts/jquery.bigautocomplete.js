	...
	var bigAutocomplete = new function(){
		...
		this.init = function(){
			...
			$("#bigAutocompleteContent").delegate("tr", "click", function() {
				...
				// 添加城市
				// bigAutocomplete.currentInputText.parent().before();
				var callback_ = bigAutocomplete.currentInputText.data("config").callback;
				if($("#bigAutocompleteContent").css("display") != "none" && callback_ && $.isFunction(callback_)){
					// modify-lee 将输入框对象传回
					var data_back = $(this).data("jsonData");
					var obj_back = {
						obj: bigAutocomplete.currentInputText,
						info: data_back
					};
					callback_(obj_back);
				}				
		...
		this.autocomplete = function(param){
			...
			var $this = $(this);//绑定自动补全功能的输入框jquery对象
			...
			// 输入框移动事件 modify-lee-4
			$(".left-inference-city-editor").scroll(function(){
				//alert(123);
				if($bigAutocompleteContent.css("display") != "none"){
					$bigAutocompleteContent.css("display", "none")
					//alert(456);
					// var offset = $this.offset();
					// console.log(offset);
					// //$bigAutocompleteContent.width(config.width);
					// var h = $this.outerHeight() - 1;
					// var lft = offset.left - 18;
					// $bigAutocompleteContent.css({"top":offset.top + h,"left":lft});
				}
			});
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(data != null && $.isArray(data) ){
						...
						for(var i=0;i<data.length;i++){
							var title = data[i].title + data[i].result.etitle;
							var reg = new RegExp(keyword_, "gi");
				...
				if(k == 13){
					...
					if($bigAutocompleteContent.css("display") != "none"){
						// modify-lee 将输入框对象传回
						var data_back = $bigAutocompleteContent.find(".ct").data("jsonData");
						var obj_back = {
							obj: $this,
							info: data_back
						};
			...
			function makeContAndShow(data_){
				...
				for(var i=0;i<data_.length;i++){
					cont += "<tr><td><div>" + data_[i].title + data_[i].result.etitle +"</div></td></tr>"
				}
				cont += "</tbody></table>";
		...
		this.hideAutocomplete = function(){
			var $bigAutocompleteContent = $("#bigAutocompleteContent");
			// modify-lee
			if(this.currentInputText != null) this.currentInputText.val("");
			if($bigAutocompleteContent.css("display") != "none"){
				$bigAutocompleteContent.find("tr").removeClass("ct");
				$bigAutocompleteContent.hide();
			}			
