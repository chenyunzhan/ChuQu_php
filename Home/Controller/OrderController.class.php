...
//        }
    ...
    public function getProductByType($id = 0, $type = '成人') {

        $type = "%".$type."%";

        $where['Classification'] = array('like',$type);
        $where['product_copy.id'] = array('eq',$id);
        $where['Price'] = array('gt',0);

        $ticket = M('product_copy');
        $lists = $ticket->join('RIGHT JOIN productticket_copy ON product_copy.productname = productticket_copy.productname')->field('
        product_copy.id as id1,
        product_copy.name,
        productticket_copy.id as id2,
        product_copy.ProductName,
        ...
        echo json_encode($lists);//json_encode方式是必须的
